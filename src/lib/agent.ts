...
export async function startGoalAgent({
  goal,
  language = 'English',
}: {
  goal: string
...
export async function analyzeTaskAgent(goal: string, task: string) {
...
export async function executeTaskAgent(
  ...
  if (analysis.action == 'search' && process.env.SERP_API_KEY) {
    ...
    if (searchResult.organic?.[0]?.snippet) {
      ...
      const summary = await summarizeSnippets(goal, analysis.arg, snippets)
